...
sealed trait Option[+A] {
  ...
  def filter(f: A => Boolean): Option[Option[A]] =
    map(a => if (f(a)) Some(a) else None)
...
object Option {
  def failingFn(i: Int): Int = {
    ...
    // A `catch` block is just a pattern matching block like the ones we've seen. `case e: Exception` is a pattern that matches any `Exception`,
  ...
  def insuranceRateQuote(age: Int, numberOfSpeedingTickets: Int): Double =
    (age * numberOfSpeedingTickets) / math.Pi
